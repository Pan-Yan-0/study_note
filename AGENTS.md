...
## 本项目教学 skill
- 2026-09-20 聊天证据画像见 [学习画像与教学适配.md](学习画像与教学适配.md)。历史整理稿须区分用户自述、AI 回答与整理者摘要；不把 AI 的讲解或“懂了”记为独立掌握。
- 教学适配：用户明确希望从神经元/具体数值理解数据传递。卡点涉及时间步与层、h/c 状态、激活、注意力分数张量和反向传播；优先“一个坐标→向量→张量→代码”，保持同一例子。不贴固定学习风格或能力标签。
...
## 进度快照（更新于用户最新进度 JSON）
- 2026-09-20 导入通义聊天整理（原对话日期未知）：用户已有门控比例的局部直觉、主动自我解释与 epoch 调参经历；仍待验证状态路径、注意力独立分数和训练闭环。文档中的 AI 修正不算用户通过验收，第十节 BERT 问题有整理错配。旧 good/not 卡点需结合此证据更新，不能据历史误解固定判断当前能力。
