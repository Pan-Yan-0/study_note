...
8. **函数必须带调用链**：教任何函数前先说明「谁调用它、参数从哪一步的哪个变量来、产出给谁用」，不允许只讲函数内部实现。
9. 中文交流，教程/页面用中文。
...
- **学习网站**（/mnt/agents/output/app，website_version_manager 发布，type=html）：
  ...
  - imdb.html **IMDB 手写实战·完整流水线**（全局调用图 + 每函数调用链标注；Part A 手写数据处理 / Part B 通用训练框架 train_model+predict_submit / Part C 六模型 M1-M6 / Part D DistilBERT 微调）
- **GitHub 仓库**：
...
- **参考书**：黄海广《斯坦福2014机器学习个人笔记》v5.52（336 页 PDF 已上传）——只精读第 9 章（反向传播）；71–87 集时对照第 10–11 章；PCA/SVM/大规模机器学习挂起备用。
...
- Kaggle notebook 可编译 C++（g++）与 CUDA 扩展（torch.utils.cpp_extension），但 GPU 需验证；当前项目瓶颈不在 Python 层。
- Kaggle 路径两种：从比赛页建 notebook 是 `/kaggle/input/competitions/xxx/`；首页自建+Add Data 是 `/kaggle/input/xxx/`。
- 导师 IMDB 脚本包：GloVe 系依赖 pickle；BERT 系独立、需联网或 hf-mirror。教程版已改为自动设备检测 + Adam。
- OOM 三连：batch_size 减半 → max_len 256 → 换 DistilBERT。

## 进度快照
- 课程：已看 1–36、42–107（共 102 集，第二部分完结）；**缺 37–41（过拟合与正则化）**，做 IMDB 过拟合实验前必须补。
- 课程策略已定：重心转向代码（IMDB 手写实战为主）；114–125 认真过（embedding 思想），108–113、126–142 快速过。
- Kaggle 作业：Part 1–3 全部完成并提交成功。
- IMDB：手写阶段（Part A→B→M1 先行）。
- 待办（用户侧）：Kaggle 手机验证、三遍法第 2 遍盲写、补看 37–41、网页上传 15 个原始脚本。
...
- 页面 head/CSS 复用：改页面时从现有页提取 <head>，通用类：.blank/.task/.shape/.tag/.chain/details。
